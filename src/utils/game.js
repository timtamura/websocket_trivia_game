...
const game = {
  prompt: {
    // current question and answer choices to be displayed
    answers: "",
    question: "",
    createdAt: "",
  },
  status: {
    submissions: {}, // players' answer submissions
    correctAnswer: "", // the correct answer to the current question
    isRoundOver: false, // whether or not the round is over.
  },
};
...
const setGameStatus = ({ event, playerId, answer, room }) => {
  if (event === "sendAnswer") {
    ...
    // ensures that only one response is stored for each player per round.
...
// get a random programming trivia question
